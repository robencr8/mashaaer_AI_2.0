...
class CosmicTheme {
  ...
  isCanvasSupported() {
    ...
    const isSupported = !!(testCanvas.getContext && testCanvas.getContext('2d'));

    if (!isSupported) {
      console.warn('Canvas is not supported by this browser. Using fallback background.');
    }
  ...
  /**
   * Apply fallback background when canvas is not supported
   */
  applyFallbackBackground() {
    ...
    document.body.classList.add('cosmic-theme-fallback');
  }
